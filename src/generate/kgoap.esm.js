...
var cloneDeep$1 = func;

var SmallestCostHolder = /*#__PURE__*/function () {
  function SmallestCostHolder(estimated_cost, cost, index) {
    _classCallCheck(this, SmallestCostHolder);

    _defineProperty(this, "estimated_cost", void 0);
  ...
  _createClass(SmallestCostHolder, null, [{
    key: "lessTo",
    value: function lessTo(a, b) {
...
function reverse_path(parents, start) {
  var path = new Array();
  var i = start;

  do {
    var _node = parents[i][0];
    path.unshift(_node);
    i = parents[i][1];
  } while (i > -1);
...
function astar(start, successors, heuristic, success) {
  var queue = new FastPriorityQueue_1(lessTo);
  queue.add(new SmallestCostHolder(0, 0, 0));
  var parents = new Array();
  parents.push([start, -1]);

  while (true) {
    var holder = queue.poll();
    if (!holder) break;
    var index = holder.index;
    var cost = holder.cost;
    var _node2 = parents[index][0];

    if (success(_node2)) {
      var path = reverse_path(parents, index);
      return [path, cost];
    }

    var neighbours = successors(_node2);

    for (var key in neighbours) {
      var neighbour = neighbours[key][0];
      var move_cost = neighbours[key][1];
      var new_cost = cost + move_cost;
      var h = heuristic(neighbour);
      var n = parents.length;
      parents.push([neighbour, index]);
      queue.add(new SmallestCostHolder(new_cost + h, new_cost, n));
    }
  }
...
var State = /*#__PURE__*/function (_Map) {
  _inherits(State, _Map);
  ...
  _createClass(State, [{
    ...
    value: function has(key) {
      return _get(_getPrototypeOf(State.prototype), "has", this).call(this, key);
    }
  }, {
    key: "get",
    value: function get(key) {
      return _get(_getPrototypeOf(State.prototype), "get", this).call(this, key);
    }
  }, {
    key: "set",
    value: function set(key, val) {
      return _get(_getPrototypeOf(State.prototype), "set", this).call(this, key, val);
    }
  }]);

  return State;
}( /*#__PURE__*/_wrapNativeSuper(Map));
var Action = function Action(name, cost, pre_conditions, post_conditions) {
  ...
  _defineProperty(this, "pre_conditions", void 0);

  _defineProperty(this, "post_conditions", void 0);
...
var PlanNode = /*#__PURE__*/function () {
  function PlanNode(current_state, action) {
    _classCallCheck(this, PlanNode);

    _defineProperty(this, "current_state", void 0);

    _defineProperty(this, "action", void 0);

    this.current_state = cloneDeep$1(current_state);
    this.action = action;
  }

  _createClass(PlanNode, [{
    key: "possible_next_nodes",
    value: function possible_next_nodes(actions) {
      var _this = this;

      var nodes = [];
      actions.forEach(function (action) {
        if (_this.matches(action.pre_conditions)) {
          nodes.push([PlanNode.child(_this.current_state, action), action.cost]);
        }
      });
      return nodes;
    }
    ...
    key: "mismatch_count",
    value: function mismatch_count(target) {
      var _this2 = this;

      var count = 0;
      target.forEach(function (target_value, name) {
        var current_state = _this2.current_state;

        if (current_state.has(name)) {
          if (current_state.get(name) !== target_value) {
            count += 1;
          }
        } else {
          count += 1;
    ...
    key: "matches",
    value: function matches(target) {
      return this.mismatch_count(target) == 0;
    }
  }], [{
    key: "initial",
    value: function initial(initial_state) {
      return new PlanNode(initial_state, undefined);
    }
  }, {
    key: "child",
    value: function child(parent_state, action) {
      var child = new PlanNode(parent_state, action);
      action.post_conditions.forEach(function (value, name) {
        child.current_state.set(name, value);
      });
      return child;
    }
  }]);
...
function plan(initial_state, goal_state, allowed_actions) {
  var start = PlanNode.initial(initial_state);
  var res = astar(start, function (node) {
    return node.possible_next_nodes(allowed_actions);
  }, function (node) {
    return node.mismatch_count(goal_state);
  }, function (node) {
    return node.matches(goal_state);
  });
  return res[0].filter(function (_, i) {
    return i > 0;
  }).map(function (v) {
...
function plan_ui(initial_state, goal_state, allowed_actions) {
  var start = PlanNode.initial(initial_state);
  var res = astar(start, function (node) {
    return node.possible_next_nodes(allowed_actions);
  }, function (node) {
  ...
  return res[0].filter(function (_, i) {
    ...
    if (v.action === undefined) {
      ...
      return [v.action.name, v.current_state];
    }
  });
}

export { Action, State, plan, plan_ui };
